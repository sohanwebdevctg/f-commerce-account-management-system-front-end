...
const UpdateTransaction: React.FC = () => {
  ...
  // ফর্ম স্টেট (প্রাথমিক ডাটা সহ)
  const [type, setType] = useState<TransactionType>(dummyTransactionData.type);
  const [amount, setAmount] = useState<string | number>(dummyTransactionData.amount);
  const [note, setNote] = useState<string>(dummyTransactionData.note || "");
  const [orderId, setOrderId] = useState<string>(dummyTransactionData.orderId || "");
  ...
  const handleUpdate = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      navigate(`/dashboard/transactions/${id || dummyTransactionData.id}`);
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-3 sm:px-4 py-6 sm:py-8">
      <div className="w-full max-w-xl bg-white rounded-2xl shadow-xl border border-gray-100 p-4 sm:p-8">
        ...
        <div className="text-center mb-6 sm:mb-8">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Update Transaction</h2>
          <p className="text-gray-500 text-xs sm:text-sm mt-1">
            Modify details and click save updates
          </p>
        </div>
          ...
          >
            {btnLoading ? (
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs sm:text-sm">Updating Transaction...</span>
              </div>
            ) : (
              <>
                <FaEdit className="text-xs sm:text-sm" />
                <span className="text-xs sm:text-sm">Update Transaction</span>
              </>
            )}
          </button>
        </form>
